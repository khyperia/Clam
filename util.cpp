...
std::string TimeEstimate::Mark(int current, int total)
{
    ...
    out << current1 << "/" << total << " (" << (int)((100.0 * current1) / total)
        << "%), " << std::setfill('0') << elapsed_min << ":" << std::setw(2)
        << elapsed_sec << " elapsed, " << time_left_min << ":" << std::setw(2)
        << time_left_sec << " left, " << total_min << ":" << std::setw(2)
        << total_sec;
    return out.str();
}
