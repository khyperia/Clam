...
{
    static __device__ Vector3<float>
    Rotate(Vector3<float> v, Vector3<float> axis, float angle)
    {
    ...
    static __device__ Vector4<float>
    TRotateD(Vector4<float> v, Vector3<float> axis, float angle)
    {
        const Vector3<float> rot = Rotate(xyz(v), axis, angle);
        return Vector4<float>(rot.x, rot.y, rot.z, v.w);
    }
    ...
    static __device__ Vector4<float>
    DeIterD(Vector4<float> z, int i, Vector3<float> offset)
    {
    ...
    Trace(Ray ray, int width, int height, Random &rand)
    {
        ...
        for (int i = 0; i < cfg.NumRayBounces; i++)
        {
            ...
            if (rand.Next() > lightingProbability)
            {
                ...
                toLight *= 1 / maxDist;
                ray = Ray(newPos, toLight);
                lighting = true;
            }
            else
            {
                Vector3<float> newDir = rand.Hemisphere(normal);

                rayColor *= dot(normal, newDir); // * surfaceColor
