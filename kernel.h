    ...
    void Set(const T &value)
    {
        if (sizeof(T) != gpuVar.bytesize())
        {
            throw std::runtime_error(
                "CPU/GPU variable size didn't match: " + tostring(sizeof(T)) + " vs. " +
                    tostring(gpuVar.bytesize()));
        }
...
public:
    ...
    void Run(int offsetX, int offsetY, int width, int height, int type, bool enqueueDownload);
};
