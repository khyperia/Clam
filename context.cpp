...
ClamContext::ClamContext()
{
    ...
    HandleErr(clGetPlatformIDs(0, 0, &openclPlatformCount));
    if (openclPlatformCount == 0)
        throw std::runtime_error("No OpenCL platforms found");

    std::vector<cl_platform_id> openclPlatforms(openclPlatformCount);
    HandleErr(clGetPlatformIDs(openclPlatformCount, openclPlatforms.data(), 0));
    cl_platform_id openclPlatform = openclPlatforms[0];

    cl_uint openclDeviceCount = 0;
    HandleErr(clGetDeviceIDs(openclPlatform, CL_DEVICE_TYPE_GPU, 0, 0, &openclDeviceCount));
